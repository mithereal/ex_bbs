defmodule Api.Stars do
  ...
  defp decrease_related_count(multi, user_id: _, post_id: post_id) when post_id != "" do
    update_post_fn = fn _repo, %{star: star} ->
      post = starrable(star)
      attrs = %{star_count: post.star_count - 1}
      Posts.update_post(post, attrs)
    end

    Multi.run(multi, :decrease_related_count, update_post_fn)
  end
  ...
  """
  @spec starrable(Star.t()) :: Topic.t() | Post.t()
  ...
  def starrable(%Star{post_id: reply_id} = star) when not is_nil(reply_id) do
    star
    |> Star.preload_reply()
    |> Map.fetch!(:reply)
  end
  ...
  defp increase_related_count(multi, %{reply_id: reply_id}) when reply_id != "" do
    update_reply_fn = fn %{star: star} ->
      ...
      Posts.update_reply(reply, attrs)
    end
    ...
    insert_notification_fn = fn _repo, %{star: star} ->
      ...
      notification_attrs =
        ...
        |> case do
          ...
          %Post{id: reply_id} -> %{reply_id: reply_id}
        end
        |> Map.merge(%{
          actor_id: author.id,
          action: action,
  ...
  # defp starrable_author(%Post{} = reply), do: Posts.author(reply)
